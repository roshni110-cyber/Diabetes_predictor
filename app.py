...
def generate_patient_report(patient_name, patient_id, result_text, patient_data, patient_photo=None, include_photo_slot=False, report_for="Patient"):
    """Create a professional 2-page medical-style diabetes report.
    Doctor report includes patient photo area. Patient self-report does not include any photo space.
    """
    ...
    glucose = float(patient_data["Glucose"].values[0])
    bmi = float(patient_data["BMI"].values[0])
    bp = float(patient_data["BloodPressure"].values[0])
    insulin = float(patient_data["Insulin"].values[0])
    skin = float(patient_data["SkinThickness"].values[0])
    age = float(patient_data["Age"].values[0])
    dpf = float(patient_data["DiabetesPedigreeFunction"].values[0])

    is_high = "High" in str(result_text)
    risk_color = red if is_high else green
    risk_label = "HIGH RISK" if is_high else "LOW RISK"
    report_mode = "Doctor Clinical Report" if include_photo_slot else "Patient Self-Check Report"

    def draw_header(title, subtitle, page_no):
        c.setFillColor(navy)
        c.rect(0, height - 82, width, 82, fill=True, stroke=False)
        c.setFillColor(cyan)
        c.rect(0, height - 85, width, 4, fill=True, stroke=False)

        c.setFillColor(white)
        c.setFont("Helvetica-Bold", 19)
        c.drawString(42, height - 35, title)
        c.setFont("Helvetica", 9)
        c.drawString(42, height - 54, subtitle)

        c.setFillColor(blue)
        c.roundRect(width - 165, height - 57, 122, 28, 9, fill=True, stroke=False)
        c.setFillColor(white)
        c.setFont("Helvetica-Bold", 10)
        c.drawCentredString(width - 104, height - 46, "GLUCOTRACK")

        c.setStrokeColor(border)
        c.line(42, 42, width - 42, 42)
        c.setFillColor(muted)
        c.setFont("Helvetica", 8)
        c.drawString(42, 28, "Generated by GlucoTrack | Screening support report | Not a medical diagnosis")
        c.drawRightString(width - 42, 28, f"Page {page_no} of 2")
    ...
    def section_label(x, y, label):
        ...
        c.setLineWidth(2)
        c.line(x, y - 5, x + 95, y - 5)

    def small_card(x, y, w, h, label, value, value_color=navy):
        c.setFillColor(white)
        c.setStrokeColor(border)
    ...
    def draw_photo(x, y, size):
        c.setFillColor(light)
        c.setStrokeColor(border)
        c.roundRect(x, y, size, size, 12, fill=True, stroke=True)
        if patient_photo:
            try:
                photo_bytes = base64.b64decode(patient_photo)
                photo_img = ImageReader(io.BytesIO(photo_bytes))
                c.drawImage(photo_img, x + 5, y + 5, size - 10, size - 10, mask="auto")
            except Exception:
                c.setFillColor(muted)
                c.setFont("Helvetica-Bold", 8)
                c.drawCentredString(x + size / 2, y + size / 2, "PHOTO")
        else:
            c.setFillColor(muted)
            c.setFont("Helvetica-Bold", 8)
            c.drawCentredString(x + size / 2, y + size / 2, "PHOTO")

    def draw_key_value(x, y, label, value):
        c.setFillColor(muted)
        c.setFont("Helvetica-Bold", 8)
    ...
    def draw_table(x, y, data_rows):
        row_h = 22
        col1 = 210
        col2 = 210
        c.setFillColor(navy)
        c.roundRect(x, y - row_h, col1 + col2, row_h, 7, fill=True, stroke=False)
        c.setFillColor(white)
        c.setFont("Helvetica-Bold", 9)
        c.drawString(x + 12, y - 15, "Clinical Input")
        c.drawString(x + col1 + 12, y - 15, "Recorded Value")

        cursor = y - row_h
        c.setFont("Helvetica", 9)
        for i, (label, value) in enumerate(data_rows):
            cursor -= row_h
            c.setFillColor(colors.HexColor("#FFFFFF") if i % 2 == 0 else colors.HexColor("#F1F5F9"))
            c.rect(x, cursor, col1 + col2, row_h, fill=True, stroke=False)
            c.setStrokeColor(border)
            c.rect(x, cursor, col1 + col2, row_h, fill=False, stroke=True)
            c.setFillColor(dark)
            c.drawString(x + 12, cursor + 7, str(label))
            c.drawString(x + col1 + 12, cursor + 7, str(value))
        return cursor

    def create_graph(title, labels, values, ylabel="Value"):
    ...
    def risk_interpretation():
        if is_high:
            return "The model has identified a higher diabetes-risk pattern. Clinical review and confirmatory testing are recommended."
        if glucose >= 100:
            return "The prediction is low-risk, but glucose is near/within the prediabetes range. Lifestyle monitoring is advised."
        return "The current prediction appears low-risk. Continue routine monitoring and healthy lifestyle practices."

    # ================= Page 1 =================
    draw_header("Diabetes Risk Screening Report", f"{report_mode} | Generated on {generated_on}", 1)

    # Patient summary card
    c.setFillColor(panel)
    c.setStrokeColor(border)
    c.roundRect(38, height - 205, width - 76, 98, 16, fill=True, stroke=True)

    c.setFillColor(dark)
    c.setFont("Helvetica-Bold", 13)
    c.drawString(58, height - 132, "Patient Summary")

    if include_photo_slot:
        # Doctor report: include photo area and shift details left
        draw_key_value(58, height - 155, "Patient Name", patient_name or "Not Provided")
        draw_key_value(58, height - 190, "Patient ID", patient_id or "Not Provided")
        draw_key_value(235, height - 155, "Report Prepared By", report_for)
        draw_key_value(235, height - 190, "Report Type", "Doctor / Enrolled Patient")
        draw_photo(width - 130, height - 191, 64)
    else:
        # Patient report: no photo and no blank photo slot
        draw_key_value(58, height - 155, "Patient Name", patient_name or "Not Provided")
        draw_key_value(58, height - 190, "Report Type", "Self-Check Patient")
        draw_key_value(270, height - 155, "Patient Email / Account", report_for if "@" in str(report_for) else "Registered Patient")
        draw_key_value(270, height - 190, "Generated On", generated_on)

    c.setFillColor(risk_color)
    c.roundRect(width - 205, height - 116, 150, 28, 9, fill=True, stroke=False)
    c.setFillColor(white)
    c.setFont("Helvetica-Bold", 11)
    c.drawCentredString(width - 130, height - 105, risk_label)

    section_label(42, height - 238, "Key Clinical Indicators")
    small_card(42, height - 310, 118, 55, "Glucose", f"{glucose:g} mg/dL", red if glucose >= 126 else amber if glucose >= 100 else green)
    small_card(174, height - 310, 118, 55, "BMI", f"{bmi:g} kg/m²", red if bmi >= 30 else amber if bmi >= 25 else green)
    small_card(306, height - 310, 118, 55, "Blood Pressure", f"{bp:g} mm Hg", red if bp >= 90 else navy)
    small_card(438, height - 310, 112, 55, "Insulin", f"{insulin:g}", amber if insulin > 200 else navy)

    section_label(42, height - 350, "Clinical Values")
    rows = [
        ("Pregnancies", patient_data["Pregnancies"].values[0]),
        ("Glucose", f"{glucose:g} mg/dL"),
        ("Blood Pressure", f"{bp:g} mm Hg"),
        ("Skin Thickness", f"{skin:g} mm"),
        ("Insulin", f"{insulin:g} μU/mL"),
        ("BMI", f"{bmi:g} kg/m²"),
        ("Diabetes Pedigree Function", f"{dpf:g}"),
        ("Age", f"{age:g} years"),
    ]
    table_bottom = draw_table(58, height - 375, rows)

    # Professional interpretation
    c.setFillColor(light)
    c.setStrokeColor(border)
    c.roundRect(42, 78, width - 84, 75, 14, fill=True, stroke=True)
    c.setFillColor(navy)
    c.setFont("Helvetica-Bold", 11)
    c.drawString(60, 130, "Clinical Interpretation")
    c.setFillColor(dark)
    c.setFont("Helvetica", 9)
    text_obj = c.beginText(60, 113)
    text_obj.setLeading(13)
    for line in [
        risk_interpretation(),
        "This report is generated from entered values and machine-learning output.",
    ...
    # ================= Page 2 =================
    draw_header("Specialist Review & Visual Summary", "Two important graphs with clinical guidance", 2)

    section_label(42, height - 125, "Important Graphs")
    graph1 = create_graph("Metabolic Risk Indicators", ["Glucose", "BMI", "Age"], [glucose, bmi, age])
    graph2 = create_graph("Pressure and Insulin Profile", ["Blood Pressure", "Insulin", "Skin"], [bp, insulin, skin])

    c.setFillColor(white)
    c.setStrokeColor(border)
    c.roundRect(42, height - 360, width - 84, 205, 14, fill=True, stroke=True)
    c.drawImage(ImageReader(graph1), 70, height - 340, 455, 160, preserveAspectRatio=True, mask="auto")

    c.setFillColor(white)
    c.setStrokeColor(border)
    c.roundRect(42, height - 590, width - 84, 205, 14, fill=True, stroke=True)
    c.drawImage(ImageReader(graph2), 70, height - 570, 455, 160, preserveAspectRatio=True, mask="auto")

    # Specialist recommendations
    section_label(42, 215, "Specialist Notes")
    if is_high or glucose >= 126:
        notes = [
            "Consult a diabetologist / physician for confirmatory tests such as FBS, PPBS and HbA1c.",
            "Reduce sugary drinks, sweets, fried foods and refined carbohydrates.",
            "Follow regular glucose monitoring and maintain a documented health routine."
        ]
        note_color = red
    elif glucose >= 100 or bmi >= 25:
        notes = [
            "Lifestyle correction is advised because glucose or BMI may indicate future risk.",
            "Prefer balanced meals with fiber, protein and controlled carbohydrate portions.",
            "Repeat screening after medical guidance and maintain regular physical activity."
        ]
        note_color = amber
    else:
        notes = [
            "Maintain healthy diet, physical activity and regular preventive checkups.",
            "Continue monitoring glucose, BMI and blood pressure periodically.",
            "Seek medical advice if symptoms or abnormal readings appear."
        ]
        note_color = green

    c.setFillColor(note_color)
    c.roundRect(42, 178, width - 84, 30, 9, fill=True, stroke=False)
    c.setFillColor(white)
    c.setFont("Helvetica-Bold", 11)
    c.drawString(58, 190, "Recommended Clinical Action")
    ...
    for i, note in enumerate(notes, 1):
        c.setFillColor(light)
        c.setStrokeColor(border)
        c.roundRect(52, y - 8, width - 104, 26, 8, fill=True, stroke=True)
        c.setFillColor(navy)
        c.setFont("Helvetica-Bold", 9)
        c.drawString(68, y, f"{i}.")
        c.setFillColor(dark)
        c.setFont("Helvetica", 9)
        c.drawString(88, y, note[:88])
        y -= 31

    c.setFillColor(muted)
    c.setFont("Helvetica-Oblique", 8)
    c.drawString(42, 55, "Disclaimer: This is a screening-support report for project/demo use. It is not a substitute for professional medical diagnosis.")
