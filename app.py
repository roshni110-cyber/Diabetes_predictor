Pasted text(48).txt
Document
change in code jo login page h usko center align karna h 

Login page ko center align karne ke liye elif menu == "🔐 Login": section me existing code replace karke ye updated structure use karo. Isme pura login card screen ke center me aayega.
...
Tumhare code me ye login section exact yahan hai:
...
Uske niche jo pura old login code hai, usko delete karo.

Phir ye naya code wahi paste karo.

👇 EXACT jagah yahi hai:

elif menu == "🔐 Login":

    # YAHA NAYA CODE PASTE KARNA HAI
...
elif menu == "🔐 Login": ko rehne do
uske niche ka purana content hatao
mera diya hua naya login code paste karo
# SIGN UP PAGE se pehle tak replace hoga
...
jo apply_theme(theme) function ke andar hai.

Uske LAST me </style> se JUST PEHLE login CSS paste karna hai.
...
Bhai toda professional aur attractive banao

Login page ko premium professional look dene ke liye ye FULL updated login section use karo.
Ye glassmorphism style, centered layout, gradient glow, better spacing aur modern UI dega.

Purana elif menu == "🔐 Login": section pura replace kar do isse:
...
# ==============================
    ...
    st.markdown("</div>", unsafe_allow_html=True)
yeh kya aa gya mere login page me

Tumne HTML ko galat jagah paste kar diya hai.
st.markdown(""" ke bahar direct <div class="login-logo"> wala code aa gaya hai, isliye woh raw text ki tarah show ho raha hai.

Ye wala part:

<div class="login-logo">🩺</div>

sirf st.markdown(""" .... """, unsafe_allow_html=True) ke andar hi hona chahiye.

Ab simple fix karo:

YE CODE REPLACE KARO
st.markdown("""
<div class="login-box">
...
Agar tumne गलती से aise likh diya:
...
toh wahi issue aa raha hai.

Aur ek aur important cheez:

Is line me unsafe_allow_html=True hona mandatory hai:

st.markdown(..., unsafe_allow_html=True)

warna Streamlit HTML ko text ki tarah print karta hai.

Ab save karo aur run karo:

streamlit run app.py

Phir login page proper professional UI me dikhega.
...
safe_caption = caption.replace("", "'").replace("\\", "\\\\") components.html(f""" <div style="margin-top:12px;"> <button id="sharePdfBtn" style=" background:linear-gradient(135deg,#16A34A,#22C55E); color:white; border:none; padding:12px 20px; border-radius:12px; cursor:pointer; font-weight:700; box-shadow:0 8px 20px rgba(34,197,94,0.25); font-family:Arial, sans-serif;"> 📎 Share PDF File </button> <p id="shareStatus" style="font-family:Arial, sans-serif; font-size:13px; color:#475569;"></p> </div> <script> const btn = document.getElementById('sharePdfBtn'); const status = document.getElementById('shareStatus'); btn.onclick = async () => {{ try {{ const b64 = "{pdf_b64}"; const byteCharacters = atob(b64); const byteNumbers = new Array(byteCharacters.length); for (let i = 0; i < byteCharacters.length; i++) {{ byteNumbers[i] = byteCharacters.charCodeAt(i); }} const byteArray = new Uint8Array(byteNumbers); const file = new File([byteArray], "{file_name}", {{type: 'application/pdf'}}); if (navigator.canShare && navigator.canShare({{ files: [file] }})) {{ await navigator.share({{ title: 'GlucoTrack Diabetes Report', text: {safe_caption}`,
files: [file]
}});
...
🩺 GLUCOTRACK
Smarter Diabetes Risk Screening
A professional machine learning dashboard for quick diabetes risk prediction, patient visualization and report generation.
...
with c1:
st.markdown(f'{total_patients}Saved Patients', unsafe_allow_html=True)
with c2:
st.markdown('MLDiabetes Risk Prediction', unsafe_allow_html=True)
with c3:
st.markdown('PDFProfessional Reports', unsafe_allow_html=True)
st.markdown("---")
a, b = st.columns(2)
with a:
